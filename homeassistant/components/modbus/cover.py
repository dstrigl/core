...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    current_status_addr = config.get(CONF_CURRENT_STATUS_ADDR)
    request_status_addr = config.get(CONF_REQUEST_STATUS_ADDR)

    add_entities([ModbusCover(
        hub, name, slave, current_status_addr, request_status_addr)]
    )
...
class ModbusCover(CoverDevice):
    ...
    def __init__(
            self, hub, name, slave,
            current_status_addr, request_status_addr):
    ...
    def open_cover(self, **kwargs):
        ...
        self._hub.write_registers(
            self._slave, self._request_status_addr, builder.to_registers()
        )
    ...
    def _set_position_and_angle(self, position, angle):
    ...
    def update(self):
        ...
        try:
            ...
            _LOGGER.error(
                "No response from hub %s, slave %s, registers [%s, %s, %s]"
                " (STATUS, POS, ANG)",
                self._hub.name,
                self._slave,
                self._current_status_addr + 0,
                self._current_status_addr + 1,
                self._current_status_addr + 2,
            )
